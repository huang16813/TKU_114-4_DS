...
class TraversalBst {
    private TraversalNode root;

    boolean add(int value) {
        if (root == null) {
            root = new TraversalNode(value);
            return true;
        }

        TraversalNode current = root;
        while (true) {
            if (value == current.value) return false;
            if (value < current.value) {
                if (current.left == null) {
                    current.left = new TraversalNode(value);
                    return true;
                }
                current = current.left;
            } else {
                if (current.right == null) {
                    current.right = new TraversalNode(value);
                    return true;
                }
    ...
    private void inorder(TraversalNode node, List<Integer> result) {
        if (node == null) return;
    ...
    private void preorder(TraversalNode node, List<Integer> result) {
        if (node == null) return;
        result.add(node.value);
    ...
    private void postorder(TraversalNode node, List<Integer> result) {
        if (node == null) return;
    ...
    private int size(TraversalNode node) {
    ...
    private int height(TraversalNode node) {
    ...
    private int leafCount(TraversalNode node) {
        if (node == null) return 0;
    ...
    String deleteCase(int target) {
        TraversalNode node = findNode(target);
    ...
    private TraversalNode findNode(int target) {
        TraversalNode current = root;
        while (current != null) {
    ...
    private TraversalNode remove(TraversalNode node, int target) {
        if (target < node.value) {
            ...
            TraversalNode successor = minimumNode(node.right);
    ...
    private TraversalNode minimumNode(TraversalNode node) {
    ...
    private void range(TraversalNode node, int low, int high,
                       List<Integer> result) {
        if (node == null) return;
    ...
    private boolean isValid(TraversalNode node, long low, long high) {
...
public class TraversalOrderReview {
    private static TraversalBst standardTree() {
    ...
    public static void main(String[] args) {
        TraversalBst tree = standardTree();
